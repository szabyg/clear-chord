...
const BASE_FREQUENCIES: Record<string, number> = NOTES.reduce((acc, note, i) => {
...
export default function TriadTuner() {
    const initialDetunes = NOTE_NAMES.reduce((acc, note) => {
        acc[note] = 0;
        return acc;
    }, {} as Record<string, number>);

    const initialActives = NOTES.reduce((acc, note) => {
        acc[note] = false;
        return acc;
    ...
    const toggleNote = (note: string) => {
        setActiveNotes((prev) => ({ ...prev, [note]: !prev[note], [note + "'"]: !prev[note + "'"] }));
    };
